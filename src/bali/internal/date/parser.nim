...
proc parseSimplifiedISO8601*(date: string): Option[float] =
  ...
  proc lexDate: bool =
    return lexYear() and (not lexer.consumeSpecific('-') or (lexMonth() and (not lexer.consumeSpecific('-') or lexDay())))

  proc lexHoursMinutes(oHours, oMinutes: var Option[int]): bool =
    var h, m: Option[int]
    if (h = lexer.lexNDigits(2); *h and &h >= 0 and &h <= 24) and lexer.consumeSpecific(':') and (m = lexer.lexNDigits(2); *m and &m >= 0 and &m <= 59):
      oHours = move(h)
      oMinutes = move(m)
      return true
  ...
  proc lexTime: bool =
    return lexHoursMinutes(hours, minutes) and (not lexer.consumeSpecific(':') or lexSecondsMilliseconds()) and lexTimezone()
  ...
  var time = dateTime(
    &year, Month(month &| 1), day &| 1, hours &| 0,
