...
proc parseIntGenerateStdIr*(vm: PulsarInterpreter, generator: IRGenerator) =
  ...
  vm.registerBuiltin("BALI_PARSEINT",
    proc(op: Operation) =
      ...
      let 
        inputString = vm.registers.callArgs[0] # 1. Let inputString be ? ToString(string).
        value = vm.trimString(inputString, TrimMode.Left) # 2. Let S be ! TrimString(inputString, start).
        ...
        radix = if vm.registers.callArgs.len > 1: # 8. If R ≠ 0, then
          ...
          if value.startsWith("0x"): # 10. a. If the length of S is at least 2 and the first two code units of S are either "0x" or "0X", then 
