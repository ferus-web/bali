  ...
  for mut in muts:
    ...
    if leftTrav.kind == IdentHolder and leftTrav.ident == mut:
      ...
      if stmt.whConditionExpr.op == BinaryOperation.LesserThan:
        debug "mutator_loops: op is less-than, optimizing away loop into the value of RHS (" & rightTrav.atom.crush() & ')'
    ...
    if rightTrav.kind == IdentHolder and rightTrav.ident == mut:
      ...
      if stmt.whConditionExpr.op == BinaryOperation.LesserThan:
        debug "mutator_loops: op is less-than, optimizing away loop into the value of LHS (" & leftTrav.atom.crush() & ')'
