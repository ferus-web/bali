...
proc generateInternalIR*(runtime: Runtime) =
  ...
  runtime.vm[].registerBuiltin(
    ...
    proc(op: Operation) =
      ...
      let
        index = uint(&getNumeric(&runtime.argument(1)))
        storeAt = uint(&getNumeric(&runtime.argument(2)))
...
proc run*(runtime: Runtime, typeRegistrationCb: proc(runtime: Runtime) = nil) =
