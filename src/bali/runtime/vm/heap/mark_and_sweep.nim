...
const
  BaliGCPointerMapSize* {.intdefine.} = 64

func hash(p: pointer): uint {.inline.} = cast[uint](p) shr 3'u32
...
proc markStack* =
  ...
  for elem in gcObject.globals:
    mark(cast[ptr uint8](elem.data.start), cast[ptr uint8](elem.data.start + elem.data.size))
...
proc mark*(start, stop: ptr uint8) =
  ...
  while start < stop:
    ...
    if idx != nil and not idx.data.marked:
      ...
      mark(cast[ptr uint8](idx.data.start), cast[ptr uint8](idx.data.start + idx.data.size))
...
proc baliMSAlloc*(size: SomeUnsignedInt): pointer =
  ...
  if (ptrVal = cast[uint](alloc(size)); ptrVal == cast[uint](nil)):
    when defined(baliGCThrowDefectOnAllocFailure):
      raise newException(AllocationError, "Failed to allocate memory chunk of size " & $size & " bytes!")
    else:
      warn "heap: failed to allocate memory chunk of size " & $size & " bytes! (Use -d:baliGCThrowDefectOnAllocFailure to make this a fatal error)"
      return
  ...
  gcObject.ptrMap[cast[uint](hash(cast[pointer](ptrVal)) mod BaliGCPointerMapSize)].add(gcPtr)
  inc gcObject.ptrNum
